...
test('bad sort', t => t.throwsAsync(fn({ q: 'bob', sort: 'bad' }), '`query.sort` should be one of "followers", "repositories" or "joined"'))
test('bad order', t => t.throwsAsync(fn({ q: 'bob', order: 'bad' }), '`query.order` should be one of "asc" or "desc"'))
test('no search', t => t.throwsAsync(fn(), errorString))
test('number search', t => t.throwsAsync(fn(666), errorString))
test('empty search', t => t.throwsAsync(fn(''), 'either `query.q` or `query.o` should be provided and not empty'))
test('bad location', t => t.throwsAsync(fn({ o: { location: 666 } }), 'location must be a string or an array of strings'))
test('bad language', t => t.throwsAsync(fn({ o: { language: 666 } }), '`query.o.language` must be a string or an array of strings'))
test('bad in', t => t.throwsAsync(fn({ o: { in: 'bad' } }), '`query.o.in` should be a string or an array of "email", "login" or "fullname"'))
test('bad in (number)', t => t.throwsAsync(fn({ o: { in: 666 } }), '`query.o.in` should be a string or an array of "email", "login" or "fullname"'))
test('bad created', t => t.throwsAsync(fn({ o: { created: 666 } }), '`query.o.created` should be a string (>=2016-03-25; <=2016-03-30; 2016-03-25..2016-03-30)'))
test('no q', t => t.throwsAsync(fn({ o: { } }), 'either `query.q` or `query.o` should be provided and not empty'))
